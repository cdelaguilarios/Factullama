...
public class GeneratePdf extends AbstractPdfView{
	...
	protected void buildPdfDocument(Map<String, Object> model,Document document, PdfWriter writer, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		float[] columnWidths3 = {5, 71, 13, 13};
		...
		for (int i = 0; i < 30; i++) {
			if(i<bill.getDetails().size()){
				...
				t3.addCell(new Paragraph(billDetail.getDescription(), new Font(Font.NORMAL,10)));
...
//		t51.getDefaultCell().setHorizontalAlignment(Element.ALIGN_LEFT);
//		t51.getDefaultCell().setBorder(0);		
//		float[] columnWidths51 = {9,16,10,65};
//		t51.setWidths(columnWidths51);
//		
//		Calendar cal = Calendar.getInstance();
//	    cal.setTime(bill.getIssueDate());
//		t51.addCell(new Paragraph(Integer.toString(cal.get(Calendar.DAY_OF_MONTH)), new Font(Font.NORMAL,11)));
//		t51.addCell(new Paragraph(Integer.toString(cal.get(Calendar.MONTH)), new Font(Font.NORMAL,11)));
//		t51.addCell(new Paragraph(Integer.toString(cal.get(Calendar.YEAR)), new Font(Font.NORMAL,11)));	
//		t51.addCell("");
		
		PdfPTable t6 = new PdfPTable (4);
		t6.setWidthPercentage(100);
		t6.getDefaultCell().setFixedHeight(22.67716535433f);	
		t6.getDefaultCell().setHorizontalAlignment(Element.ALIGN_LEFT);
